...
Then in the current directory place a replay buffer .npz file with the name `human_demos.npz`. The script will translate this and output `translated_demos.npz`.
...
import numpy as np
import os
import pickle as npz
import torch
from data import create_dataset
from models import create_model
from options.test_options import TestOptions
from real_robots.utils.replay_buffer import load_replay_buffer
...
def get_output_npz_filename(demo_filename):
    """Returns a filename for the output pickle file for the updated replay buffer.
    Example: red-cube-grasping-robot-demos.npz -> red-cube-grasping-robot-demos_cropped-out-fingers.npz"""
    split_list = demo_filename.split('.npz') # Example: 'red-cube-grasping-robot-demos.npz' -> ['red-cube-grasping-robot-demos', '']
    assert len(split_list) == 2
    return split_list[0] + '_cyclegan-translated.npz'
...
if __name__ == '__main__':
    ...
    if not os.path.isfile(demo_file_name):
        ...
        exit(0)
    replay_buffer = load_replay_buffer(demo_file_name)
    ...
    output_file_name = get_output_npz_filename(demo_file_name)
    replay_buffer.save(output_file_name)
